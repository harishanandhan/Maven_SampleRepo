...
public class AutomationReport implements IReporter {

	private static final Logger LOG = Logger.getLogger(AutomationReport.class);
	...
	private String fileName = "TestAutomationReport.html";
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		boolean mkdirs = new File(outdir).mkdirs();
	...
	protected void writeStylesheet() {
		...
		writer.print("th {vertical-align:bottom;background-color: #CCC;}");
		...
		writer.print(".stacktrace {white-space:pre;font-family:Arial, sans-serif}"); // Changed font to Arial
	...
	protected String getFormattedStartTime(long startTimeInMillisFromEpoch) {
		...
		Date date = new Date(startTimeInMillisFromEpoch);
	...
	protected void writePieChart() {
		...
		writer.println("<div style=\"position: absolute; top: 10px; right: 10px;\">");

		writer.println("<canvas id=\"testPieChart\" width=\"200\" height=\"200\"></canvas>");

		writer.println("</div>");

		writer.println("<script>");

		writer.println("var ctx = document.getElementById('testPieChart').getContext('2d');");

		writer.println("var myPieChart = new Chart(ctx, {");
		...
		writer.println("data: [" + totalPassedTests + ", " + totalFailedTests + ", " + totalSkippedTests + "],");

		writer.println("backgroundColor: ['#3F3', '#F33', '#DDD']");
		...
		writer.println("responsive: false,");
